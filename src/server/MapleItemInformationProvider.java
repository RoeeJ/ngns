...
public class MapleItemInformationProvider {
    ...
    public static Pair<Integer, String> getItemPairById(int itemId) {
        try {
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT `id`, `itemid`, `itemname` FROM `items` WHERE `itemid` = ?")) {
                ps.setInt(1, itemId);
    ...
    public static Pair<Integer, String> getItemPairByName(String itemName) {
        try {
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT `id`, `itemid`, `itemname` FROM `items` WHERE `itemname` = ?")) {
    ...
    public static List<Pair<Integer, String>> getItemPairByNameLike(String itemName) {
        try {
            ArrayList<Pair<Integer, String>> pairs = new ArrayList<>();
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT `id`, `itemid`, `itemname` FROM `items` WHERE `itemname` LIKE ?")) {
                ps.setString(1, "%" + itemName + "%");
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        int itemId = rs.getInt("itemid");
                        itemName = rs.getString("itemname");
                        pairs.add(new Pair<>(itemId, itemName));
                    }
                }
    ...
    public String getName(int itemId) {
        try {
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM items WHERE `itemid`=?")) {
                ps.setInt(1, itemId);
                try (ResultSet rs = ps.executeQuery()) {
