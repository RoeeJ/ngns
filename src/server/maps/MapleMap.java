...
public class MapleMap {
    ...
    protected boolean diseaseMap = false;
    ...
    public void broadcastMessage(MapleCharacter source, final byte[] packet) {
        ...
        try {
            ...
            FilePrinter.printError(FilePrinter.LOGGER + this.getClass().getName() + ".txt", e);
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
        } finally {
            chrRLock.unlock();
        }
    }
...
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
    ...
    public boolean DiseaseMap(final Pair<MapleDisease,MobSkill> mdms)
    {
    	if(diseaseThread != null || diseaseMap){
    		return false;
    	}
    	diseaseThread = new Thread(() -> {
            try{
            while(getDiseaseMap()){
                getCharacters().stream().filter(chr->!chr.isGM()).forEach(chr -> chr.giveDebuff(mdms.getLeft(), mdms.getRight()));
                Thread.sleep(1000);
            }
            }
    ...
    public boolean getDiseaseMap(){return diseaseMap;}
...
//        throw new RuntimeException("Out of OIDs on map " + mapid + " (channel: " + channel + ")");
    ...
    public void removeMapObject(int num) {
        objectWLock.lock();
        try {
            FilePrinter.printError(FilePrinter.DEBUG, "Remove map object with ID: " + num);
...
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
            ...
            if (Randomizer.nextInt(999999) < de.chance * chServerrate) {
                ...
                if (de.itemId != 0) {
                    if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                        ...
                        idrop = new Item(de.itemId, (byte) 0, (short) (de.Maximum != 1 ? Randomizer.nextInt(Math.abs(de.Maximum - de.Minimum)) + de.Minimum : 1));
                    }
        ...
        int mesos = Randomizer.nextInt(mob.getLevel()*100 - mob.getLevel()*10) + mob.getLevel()*10;
        ...
        for (final MonsterGlobalDropEntry de : globalEntry) {
            if (Randomizer.nextInt(999999) < de.chance) {
                ...
                if (de.itemId == 0) {
                    chr.getCashShop().gainCash(1, Randomizer.nextInt(10*mob.getLevel()));
                } else {
                    if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                        idrop = ii.randomizeStats((Equip) ii.getEquipById(de.itemId));
                    } else {
    ...
    private void spawnDrop(final Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, final byte droptype, final short questid) {
        ...
        if (items.size() > 256) {
            removeMapObject(items.get(0));
    		broadcastMessage(MaplePacketCreator.removeItemFromMap(items.get(0).getObjectId(), 0, 0));
    		items.remove(0);
    	}
...
//                Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
    ...
    public void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean secondTime, int animation) {
        ...
        if (withDrops && !monster.dropsDisabled()) {
            ...
            if(!monster.isBoss() && Randomizer.nextBoolean() && Randomizer.nextBoolean()){
...
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
    ...
    public void updateMonsterController(MapleMonster monster) {
        ...
        try {
            if (!monster.isAlive()) {
//                FilePrinter.printError(FilePrinter.DEBUG + this.getClass().getName() + ".txt", "B: Remove monster: " + monster.getId() + " from Map ID: " + getId());
//                removeMapObject(monster);
                return;
            }
            ...
            try {
                for (MapleCharacter chr : characters) {
                    if (!chr.isHidden() && !chr.isInvisible() && (chr.getControlledMonsters().size() < mincontrolled || mincontrolled == -1)) {
...
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
    ...
    public int getRandomCharacter(){
    	ArrayList<MapleCharacter> chars = (ArrayList<MapleCharacter>) getCharacters();
    	Random rnd = new Random();
    	return chars.get(rnd.nextInt(chars.size())).getId();
    }
    ...
    public final void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final Item item, Point pos, final boolean ffaDrop, final boolean playerDrop) {
        ...
        spawnAndAddRangedMapObject(drop, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                ...
                if (chr != null && chr.getNTI()) {
                    chr.getMap().broadcastMessage(MaplePacketCreator.getChatText(chr.getId(), MapleItemInformationProvider.getItemPairById(item.getItemId()).getRight(), false, 0));
                }
    ...
    private void dropRoll() {
        List<MapleMapObject> items = getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY, Arrays.asList(MapleMapObjectType.ITEM));
        if (items.size() > 256) {
            broadcastMessage(MaplePacketCreator.removeItemFromMap(items.get(0).getObjectId(), 0, getRandomCharacter()));
    		removeMapObject(items.get(items.size()-1));
    	}
    	items = null;
	}
...
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
        ...
        for (int i = 0; i < chr.getPets().length; i++) {
            try {
                if (pets[i] != null) {
                    // Weird syntax because of debugging (NullPointerException)
                    pets[i].setPos(
                            getGroundBelow(
                                    chr.getPosition()
                            )
        ...
        } else if (chr.isInvisible()) {
            broadcastGuardMessage(chr, MaplePacketCreator.spawnPlayerMapobject(chr), false);
            chr.announce(MaplePacketCreator.getGMEffect(0x10, (byte) 1));
        } else {
...
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
        ...
        if (chr.isInvisible()) {
            broadcastGuardMessage(MaplePacketCreator.removePlayerFromMap(chr.getId()));
        } else if (chr.isHidden()) {
    ...
    public void broadcastGuardMessage(final byte[] packet) {
        broadcastGuardMessage(null, packet, Double.POSITIVE_INFINITY, null);
    }
...
//            Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, e.getMessage(), e);
    ...
    public void broadcastGMMessage(final byte[] packet, Point rangedFrom) {
        broadcastGMMessage(null, packet, 722500, rangedFrom);
    }
    ...
    public void broadcastGuardMessage(MapleCharacter source, final byte[] packet, boolean repeatToSource) {
        broadcastGuardMessage(repeatToSource ? null : source, packet, Double.POSITIVE_INFINITY, source.getPosition());
    }

    private void broadcastGuardMessage(MapleCharacter source, final byte[] packet, double rangeSq, Point rangedFrom) {
        chrRLock.lock();
        try {
            for (MapleCharacter chr : characters) {
                if (chr != source && chr.isGuard()) {
                    if (rangeSq < Double.POSITIVE_INFINITY) {
                        if (rangedFrom.distanceSq(chr.getPosition()) <= rangeSq) {
                            chr.getClient().announce(packet);
                        }
    ...
    public void clearDrops(MapleCharacter player) {
        ...
        player.dropMessage(String.valueOf(items.size()));
    ...
    public SpeedTyper getSpeedTyper() {
        return speedTyper;
    }
