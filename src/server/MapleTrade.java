...
public class MapleTrade {
    ...
    public void addItem(Item item) {
    	if(!ItemConstants.isRechargable(item.getItemId()) && item.getQuantity() == 0) {
    		return;
    	}
        items.add(item);
    ...
    public void chat(String message) {
        ...
        ChatLog.getInstance().add("["+ChatLog.getInstance().generateTime()+"]" + " [Trade] " + chr.getClient().getPlayer().getName() + ": " + message);
