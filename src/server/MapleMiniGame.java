...
public class MapleMiniGame extends AbstractMapleMapObject {
    ...
    public void sendOmok(MapleClient c, int type) {
        c.announce(MaplePacketCreator.getMiniGame(this, isOwner(c.getPlayer()), type));
    }

    public void sendMatchCard(MapleClient c, int type) {
        c.announce(MaplePacketCreator.getMatchCard(this, isOwner(c.getPlayer()), type));
    }
    ...
    public void setPiece(int move1, int move2, int type, MapleCharacter chr) {
        ...
        if (piece[slot] == 0) {
            piece[slot] = type;
            this.broadcast(MaplePacketCreator.getMiniGameMoveOmok(move1, move2, type));
            for (int y = 0; y < 15; y++) {
                for (int x = 0; x < 11; x++) {
