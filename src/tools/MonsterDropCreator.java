...
public class MonsterDropCreator {
	...
	private static boolean includeQuestDrops; // Use for quest drop for my
												// server, ignore.
	protected static String monsterQueryData = "monster_dropz"; // Modify this
																// to suite your
																// source
	...
	public static void main(String args[]) throws FileNotFoundException,
			IOException, NotBoundException, InstanceAlreadyExistsException,
		...
		for (MapleData dataz : data.getChildren()) {
			...
			if (dataz.getChildByPath("reward").getChildren().size() > 0) { // Fix
				...
				for (MapleData drop : dataz.getChildByPath("reward")) {
					...
					if (first) {
						sb.append("(DEFAULT, " + idtoLog + ", " + itemid + ", "
								+ rate);
		...
		for (Pair<Integer, String> Pair : itemNameCache) {
			if (Pair.getLeft() >= 2380000 && Pair.getLeft() <= 2388054) {
				...
				for (Pair<Integer, MobInfo> Pair_ : mobCache) {
					if (Pair_.getRight().getName()
							.equalsIgnoreCase(bookName.toString())) {
						int rate = 7000;
						if (Pair_.getRight().getBoss() > 0) {
							rate /= 10;
						}
						SQL.append("INSERT INTO " + monsterQueryData
								+ " VALUES (DEFAULT, " + Pair_.getLeft() + ", "
								+ Pair.getLeft() + ", " + rate);
						if (includeQuestDrops) {
							SQL.append(", DEFAULT);");
						} else {
		...
		// Extra, not done.
		/*
		 * Properties dbProp = new Properties(); FileReader fileReader = new
		 * FileReader("db.properties"); dbProp.load(fileReader);
		 * fileReader.close(); DatabaseConnection.setProps(dbProp);
		 * DatabaseConnection.getConnection();
		 * 
		 * try { PreparedStatement ps =
		 * DatabaseConnection.getConnection().prepareStatement
		 * ("SELECT * FROM monstercarddata"); ResultSet rs = ps.executeQuery();
		 * 
		 * StringBuilder SQLz = new StringBuilder(); FileOutputStream outbook =
		 * new FileOutputStream("book.sql", true); int mob, cardid, rate; while
		 * (rs.next()) { rate = 7000; mob = rs.getInt("mobid"); cardid =
		 * rs.getInt("cardid");
		 * 
		 * for (Pair<Integer, MobInfo> Pair : mobCache) { if (Pair.getLeft() ==
		 * mob) { if (Pair.getRight().getBoss() > 0) { rate /= 10; }
		 * SQLz.append("INSERT INTO " + monsterQueryData + " VALUES (DEFAULT, "
		 * + mob + ", " + cardid + ", " + rate + ");"); SQLz.append("\n");
		 * break; } } } outbook.write(SQLz.toString().getBytes()); } catch
		 * (SQLException e) { }
		 */
		...
		System.out.println("b00~ You've wasted " + time
				+ " seconds of your life... Sucks for ya");
	}
	...
	private static void getAllItems() {
		...
		MapleDataProvider data = MapleDataProviderFactory
				.getDataProvider(new File(System
						.getProperty("wzpath") + "/String.wz"));
	...
	public static void getAllMobs() {
		...
		for (MapleData itemFolder : mob.getChildren()) { // Get the list of mobs
			...
			try {
				MapleData monsterData = mobData.getData(StringUtil
						.getLeftPaddedStr(Integer.toString(id) + ".img", '0',
								11));
				MobInfo mobInfo = new MobInfo(id == 8810018 ? 1
						: MapleDataTool.getIntConvert("boss",
								monsterData.getChildByPath("info"), 0), // fix
																		// for
																		// HT
						MapleDataTool.getIntConvert("rareItemDropLevel",
								monsterData.getChildByPath("info"), 0), // New
																		// info
																		// string
																		// added
																		// since
