...
public final class UseCashItemHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c, int header) {
        ...
        if (System.currentTimeMillis() - player.getLastUsedCashItem() < 3000 || c.getPlayer().getJailed()) {
            return;
        }
...
//                if(!player.getJob().isA(MapleJob.getById(SPTo/10000),true)) return;
            ...
            switch (itemId / 1000 % 10) {
                case 1: // Megaphone
                    if (player.getLevel() > 9) {
                        ...
                        ChatLog.getInstance().add("[" + ChatLog.getInstance().generateTime() + "]" + " [Mega] " + c.getPlayer().getName() + ": " + slea.readMapleAsciiString());
                ...
                case 2: // Super megaphone
                    ...
                    ChatLog.getInstance().add("[" + ChatLog.getInstance().generateTime() + "]" + " [Super] " + c.getPlayer().getName() + ": " + slea.readMapleAsciiString());
                    if (ChatLog.getInstance().getChat().size() >= 1)
                        ChatLog.getInstance().makeLog(); //change 500 to w/e
                    break;
                case 5: // Maple TV
                ...
                case 6: //item megaphone
                    ...
                    ChatLog.getInstance().add("[" + ChatLog.getInstance().generateTime() + "]" + " [Item] " + c.getPlayer().getName() + ": " + msg);
                    if (ChatLog.getInstance().getChat().size() >= 1)
                        ChatLog.getInstance().makeLog(); //change 500 to w/e
                    break;
            ...
            player.getMap().broadcastMessage(player, MaplePacketCreator.changePetName(player, newName), true);
            c.announce(MaplePacketCreator.enableActions());
            remove(c, itemId);
            ...
            if (!vip) {
                ...
                if (c.getChannelServer().getMapFactory().getMap(mapId).getForcedReturnId() == 999999999 && (c.getPlayer().isTrockMap(mapId) || c.getPlayer().isVipTrockMap(mapId))) {
            ...
            ChatLog.getInstance().add("[" + ChatLog.getInstance().generateTime() + "]" + " [Messenger] " + c.getPlayer().getName() + ": " + lines);
            if (ChatLog.getInstance().getChat().size() >= 1)
                ChatLog.getInstance().makeLog(); //change 500 to w/e break;
            remove(c, itemId);
