    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) {// trade
                ...
                if (!(chr.getGMRank() == GMRank.DEVELOPER)) {
            ...
            MapleMiniGame game = chr.getMiniGame();
            game.broadcast(MaplePacketCreator.getMiniGameReady());
        } else if (mode == Action.UN_READY.getCode()) {
            MapleMiniGame game = chr.getMiniGame();
            game.broadcast(MaplePacketCreator.getMiniGameUnReady());
        } else if (mode == Action.START.getCode()) {
            MapleMiniGame game = chr.getMiniGame();
            if (game.getGameType().equals("omok")) {
                game.broadcast(MaplePacketCreator.getMiniGameStart(game.getLoser()));
            ...
            MapleMiniGame game = chr.getMiniGame();
            if (game.isOwner(c.getPlayer())) {
                game.broadcastToVisitor(MaplePacketCreator.getMiniGameRequestTie());
            } else {
                game.getOwner().getClient().announce(MaplePacketCreator.getMiniGameRequestTie());
            }
            ...
            MapleMiniGame game = chr.getMiniGame();
            if (game.isOwner(c.getPlayer())) {
                game.broadcast(MaplePacketCreator.getMiniGameSkipOwner());
            } else {
                game.broadcast(MaplePacketCreator.getMiniGameSkipVisitor());
            }
            ...
            if (turn == 1) {
                ...
                if (game.isOwner(c.getPlayer())) {
                    ...
                    game.getOwner().getClient().announce(MaplePacketCreator.getMatchCardSelect(turn, slot, firstslot, turn));
                }
